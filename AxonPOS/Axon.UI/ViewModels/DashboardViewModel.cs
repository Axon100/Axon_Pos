            ...
            try
            {
                ...
                switch (SelectedDateRange)
                {
                    ...
                    case "كافة الأوقات":
                        ...
                        DateRange = "كافة الفترات والعمليات المسجلة";
                        break;
                    default:
                        startDate = DateTime.Today.AddDays(-29);
                        DateRange = $"{startDate.ToString("d MMMM", arCulture)} - {now.ToString("d MMMM yyyy", arCulture)}";
                ...
                var filteredSales = allSales.Where(s => s.Date >= startDate).ToList();
                ...
                var periodExpenses = allExpenses.Where(e => e.ExpenseDate.DateTime >= startDate).ToList();
                var periodReturns = allReturns.Where(r => r.ReturnDate.DateTime >= startDate).ToList();
                ...
                int daysCount = SelectedDateRange switch
                {
                    "اليوم" => 1,
                    "آخر 7 أيام" => 7,
                    "آخر 30 يوم" => 14, // 14 representative intervals for clean UI
                    _ => 7
                };
